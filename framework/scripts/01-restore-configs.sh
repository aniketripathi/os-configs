...
# Argument Parsing
if [[ $# -eq 0 ]]; then
    # Default behavior: restore both configs and keys
...
if $RESTORE_KEYS; then
    echo "=== Restoring Private Keys and Git Identity ==="

    # 1. Restore private gitconfig if it exists in keys/
    if [[ -f "$KEYS_DIR/gitconfig" ]]; then
        echo "Restoring private gitconfig from $KEYS_DIR/gitconfig..."
        cp "$KEYS_DIR/gitconfig" "${HOME}/.gitconfig"
    fi

    # 2. Restore SSH keys if they exist in keys/ssh/
    if [[ -d "$KEYS_DIR/ssh" ]]; then
        echo "Restoring SSH keys from $KEYS_DIR/ssh/..."
        mkdir -p "${HOME}/.ssh"
        chmod 700 "${HOME}/.ssh"
        cp "$KEYS_DIR/ssh/id_ed25519"* "${HOME}/.ssh/"
